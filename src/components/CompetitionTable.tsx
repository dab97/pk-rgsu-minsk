            ...
            {isColMenuOpen && (
              <>
                ...
                <div className="absolute right-0 top-full mt-2 w-60 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-xl p-2.5 z-50 flex flex-col gap-1">
                  ...
                  {/* Пункты меню соответствуют колонкам текущей основы:
                      «Оплата» есть только на Платном, «Высшие приоритеты» — только на Бюджете */}
                  {COLS.filter(({ key }) =>
                    (key !== 'semesterPayment' || activeBasis === 'Платное') &&
                    (key !== 'higherPriority' || activeBasis === 'Бюджет')
                  ).map(({ key, label }) => (
